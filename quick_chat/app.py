...
def register():
    if request.method == "POST":
        ...
        # If email was provided, send a verification OTP BEFORE saving the account.
        # The account is only written to disk after OTP is confirmed in verify_email().
        if email:
            otp = _generate_otp(6)
            ...
            # Store the pending user data in the session — do NOT write to users.json yet
            ...
            if email_sent:
                flash("Verification code sent to your email. Please verify it to complete registration.", "info")
            else:
                flash("Verification code: " + otp, "info")

            return redirect(url_for("verify_email"))

        # No email — create account immediately
        users[uid] = pending_user
        write_json(USERS_FILE, users)
...
def verify_email():
    ...
    if request.method == "POST":
        ...
        if pending_user:
            # Account was held in session — only now write it to disk
            pending_user["email_verified"] = True
            users[uid] = pending_user
            session.pop("pending_user", None)
        elif uid in users:
            # Re-verification of an existing account
